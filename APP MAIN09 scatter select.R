      ...
      div(class="outer",
          ...
          absolutePanel(
            ...
            selectizeInput(
              "sport", 
              "Select Sport",
              choices = unique(summer_olympic$Sport),
              multiple = TRUE,
              options = list(
          ...
          absolutePanel(
            ...
            top = 75, right = 55, width = 1000, fixed = TRUE,
            draggable = TRUE, height = "auto",
            ...
            span(tags$i(h6("By analyzing the choropleth, we can gain insights into 
                           how the attribute distribution of successful athletes differs among countries.", align = "justify")), style="color:#045a8d")
            
          ),
          
          absolutePanel(
            id = "histogram", class = "panel panel-default",
            top = 1000, right = 30, width = 500, fixed=TRUE,
            draggable = TRUE, height = "auto",
            
            span(tags$i(h6("The Age x Medal plot reveals that there exists a specific age range where athletes tend to excel and secure medals,
               ...
               Moreover, by segregating the plot based on different sports, we can identify the golden age specific to each sport.")), style="color:#045a8d"),
...
server = function(input, output, session) {
  ...
  output$line_host <- renderPlotly({
    ...
    chosen_data_host <- data.frame(Year = filtered_host$Year,
                                   City = filtered_host$City,
                                   NOC = filtered_host$NOC)
    
    # set data to be shown as selected by selectinput
    chosen_data_host$diff_data <- case_when(
  ...
  output$scatter_host <- renderPlotly({
    ...
    chosen_data_country <- data.frame(NOC = filtered_country$NOC,
                                      Year = filtered_country$Year)
    
    # set countries data to be shown as selected by selectinput
    chosen_data_country$data_country <- case_when(
    ...
    # filter host data based on data type
    chosen_data_host <- data.frame(Year = filtered_host$Year,
                                   City = filtered_host$City,
                                   NOC = filtered_host$NOC)
    
    # set as host data to be shown as selected by selectinput
    chosen_data_host$data_as_host <- case_when(
    ...
    # set before host data to be shown as selected by selectinput
    chosen_data_host$data_b_host <- case_when(
    ...
    # set average data to be shown as selected by selectinput
    chosen_data_host$data_avg <- case_when(
    ...
    # set differemce data to be shown as selected by selectinput
    chosen_data_host$diff_data <- case_when(
      input$rank == "Bronze Count" ~ filtered_host$cbronze_diff,
      input$rank == "Silver Count" ~ filtered_host$csilver_diff,
      input$rank == "Gold Count" ~ filtered_host$cgold_diff,
    ...
    plot_ly(data = chosen_data_country,
      ...
      add_trace(
        ...
        color = I("darkgrey"),  opacity = 0.6, type = "scatter", mode = "markers",
