...
host_country$rbronze_diff <- host_country$bronzerank_b_host - host_country$bronzerank_as_host
host_country$rsilver_diff <- host_country$silverrank_b_host - host_country$silverrank_as_host
host_country$rgold_diff <- host_country$goldrank_b_host - host_country$goldrank_as_host
host_country$rmedal_diff <- host_country$medalrank_b_host - host_country$medalrank_as_host

# gold per country in each games
gold_per_country <- summer_olympic %>%    # Applying group_by & summarise
  group_by(NOC, Year) %>%
...
medal_per_country <- medal_per_country %>%    # Applying group_by & summarise
  group_by(Year) %>%
  mutate(country_rank = ceiling(rank(-medal_count)))
...
server = function(input, output, session) {
  ...
  output$line_host <- renderPlotly({
    ...
    plot_ly(chosen_data_host, x = ~Year, y = ~diff_data, type = "scatter",
            mode = "lines+markers", group = "group", marker = list(color = ~ifelse(diff_data >= 0, "darkgreen", "red"),width = 2),
